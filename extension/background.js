...
async function handleUrlCheck(url) {
  try {
    ...
    if (
      cached[cacheKey] &&
      Date.now() - cached[cacheKey].timestamp < CACHE_TTL
    ) {
    ...
    if (!response.ok) {
      ...
      if (!fallbackResponse.ok) {
        throw new Error(
          `Fallback validation failed with status ${fallbackResponse.status}`,
        );
      }
      ...
      const trustScore =
        serverResult.trustScore !== undefined ? serverResult.trustScore : 100;
      ...
      const isMalicious =
        threatScore > 70 || forensicSignals.isSuspiciousRegistrar;
    ...
    chrome.storage.local.get(
      ...
      async (settings) => {
        ...
        if (result && result.status !== "error") {
          ...
          const isSuspicious =
            result.status === "suspicious" || (score > 20 && score <= 70);
