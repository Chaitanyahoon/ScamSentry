...
const mockSafeCompanies: SafeCompany[] = [
  {
    ...
    description:
      "Highly reputable agency known for clear communication and timely payments.",
  ...
  },
  {
    ...
    description:
      "Praised by freelancers for organised projects and competitive rates.",
    ...
    const fetchSafeCompanies = async () => {
      ...
      try {
        const backendUrl =
          ...
          "https://scamsentry-backend-j7a8.onrender.com";
        console.log(
          `[SAFE_COMPANIES] Attempting to fetch brand lockdowns from FastAPI backend: ${backendUrl}`,
        );
        ...
        if (res.ok) {
          ...
          if (data && data.length > 0) {
            ...
            console.log(
              `[SAFE_COMPANIES] Successfully loaded ${mapped.length} brand lockdowns from FastAPI.`,
            );
          }
        }
      } catch (err) {
        console.error(
          "[SAFE_COMPANIES] Failed to fetch brand lockdowns from backend, falling back to Firestore:",
          err,
        );
      }
        ...
        <div className="mx-auto max-w-7xl">
          <div className="flex flex-col items-center mb-12 text-center">
            ...
            <p className="mt-4 text-sm text-muted-foreground max-w-xl">
              A community-curated directory of safe, verified clients and
              organizations known for transparent contracting and reliable
              payments.
            </p>
          </div>
          ...
          <div className="bg-card border border-border rounded-2xl shadow-lg mb-10 overflow-hidden">
            <div className="bg-card border-b border-border px-6 py-4">
              <div className="flex items-center gap-2 text-sm font-semibold text-foreground">
                <Building className="h-4.5 w-4.5 text-success" /> Verified
                Directory Ledger
              </div>
            </div>
              ...
              {isLoading ? (
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                  {companies.map((company) => (
                    <div
                      key={company.id}
                      className="flex flex-col justify-between bg-card border border-border hover:border-success/45 rounded-2xl overflow-hidden shadow-sm hover:shadow-md transition-all duration-300"
                    >
                      <div className="p-6 border-b border-border/50 bg-card/40">
                        <div className="flex items-start justify-between mb-2">
                          <h2 className="text-lg font-bold text-foreground capitalize truncate max-w-[75%]">
                            {company.name.toLowerCase()}
                          </h2>
                          <Badge
                            variant="outline"
                            className="flex items-center gap-1 text-xs px-2.5 py-0.5 rounded-full border-success/30 bg-success/10 text-success"
                          >
                          ...
                          <div className="flex flex-wrap gap-2 text-xs">
                            {company.tags.map((tag) => (
                              <Badge
                                key={tag}
                                variant="outline"
                                className="rounded-full bg-card border-border px-2.5 py-0.5 text-muted-foreground"
                              >
                                #{tag.replace(/\s+/g, "-")}
                              </Badge>
                            ))}
                  ...
                  {companies.length === 0 && (
                    <div className="col-span-full border border-border p-12 text-center bg-card/50 rounded-2xl">
                      ...
                      <p className="text-muted-foreground text-sm">
                        No verified organizations found in the database.
                      </p>
                    </div>
                  )}
                </div>
