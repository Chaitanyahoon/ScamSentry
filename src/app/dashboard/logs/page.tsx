...
const INITIAL_LOGS: LogEntry[] = [
  { id: "1", timestamp: "18:12:05", source: "ENGINE", type: "INFO", message: "ScamSentry Sentinel core initialized. 4 active layers functional." },
  { id: "2", timestamp: "18:12:06", source: "DNS", type: "SUCCESS", message: "SPF resolver operational. DNS txt records mapped to promise handlers." },
  { id: "3", timestamp: "18:12:07", source: "SCRAPER", type: "INFO", message: "Cron scheduling verified. GitHub Actions hourly sync trigger active." },
  { id: "4", timestamp: "18:12:10", source: "VALIDATOR", type: "INFO", message: "Checking threat domain blocklists (total hashes loaded: 42,891)." },
  { id: "5", timestamp: "18:12:15", source: "DNS", type: "SUCCESS", message: "SPF PASS: github.com [v=spf1 include:_spf.github.com ~all]" },
  { id: "6", timestamp: "18:12:18", source: "DNS", type: "SUCCESS", message: "DMARC PASS: github.com [v=DMARC1; p=reject; pct=100; rua=mailto:d@github.com]" },
  { id: "7", timestamp: "18:12:22", source: "SCRAPER", type: "SUCCESS", message: "Daily cyber bulletin ingest complete. Pinned 2 highlighted incidents." }
];

const LOG_MESSAGES = [
  { source: "DNS", type: "WARN", message: "SPF MISMATCH: domain vercel-security-alert.net has no active TXT record configuration." },
  { source: "VALIDATOR", type: "CRITICAL", message: "🚨 LOCKDOWN DETECTED: mimicked brand 'GITHUB' detected in domain 'github-login-support.xyz'. Applied +35 risk penalty score." },
  { source: "ENGINE", type: "INFO", message: "Analyzing inbound request fingerprint telemetry cache. Deduplication index: 98.4%" },
  { source: "VALIDATOR", type: "WARN", message: "Burner certificate match found for paytm-refund-dossier.in. Elevated risk index (+20)." },
  { source: "DNS", type: "SUCCESS", message: "DMARC PASS: vercel.com [v=DMARC1; p=reject; rua=mailto:dmarc@vercel.com]" },
  { source: "SCRAPER", type: "INFO", message: "Fetching BleepingComputer RSS feeds... Parsed 15 new OSINT packets." },
  { source: "ENGINE", type: "SUCCESS", message: "Consensus validation matrix synchronized successfully across global nodes." },
  { source: "VALIDATOR", type: "CRITICAL", message: "🚨 LOCKDOWN DETECTED: mimicked brand 'VERCEL' detected in domain 'vercel-deployments.top'. Applied +35 risk penalty score." }
] as const;
...
export default function ForensicLogsPage() {
  ...
  const [sourceFilter, setSourceFilter] = useState<"all" | "ENGINE" | "SCRAPER" | "VALIDATOR" | "DNS">("all");
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 border-b border-[#1F1914] pb-6">
        <div>
          ...
          <p className="text-sm text-muted-foreground mt-1">
            Real-time scrolling debugger feed capturing DNS forensics, OSINT syncs, and brand lockdowns.
          </p>
        </div>
        ...
        <div className="flex gap-2">
          <Button
            ...
            className={cn(
              ...
              isPlaying ? "text-amber-500 hover:text-amber-400" : "text-emerald-500 hover:text-emerald-400"
            )}
          >
            {isPlaying ? (
              <>
        ...
        <div className="bg-[#0C0A09] p-3 border-b border-[#1F1914] flex items-center justify-between">
          ...
          <div className="flex items-center gap-3">
            <div className="flex items-center gap-1 text-[9px] font-mono text-muted-foreground/50">
              <span className={cn("h-1.5 w-1.5 rounded-full bg-emerald-500 mr-1.5", isPlaying ? "animate-pulse" : "opacity-50")}></span>
              STATUS: {isPlaying ? "SCROLLING" : "HALTED"}
            </div>
            <span className="text-[9px] font-mono text-muted-foreground/30">|</span>
            <span className="text-[9px] font-mono text-muted-foreground/50">BUFFER: {filteredLogs.length} LOGS</span>
          </div>
        </div>
        ...
        <div className="p-6 h-[480px] overflow-y-auto space-y-3 select-text font-mono text-[11px] leading-relaxed">
          {filteredLogs.length === 0 ? (
            ...
            filteredLogs.map(log => (
              ...
              >
                {/* Time stamp */}
                <span className="text-muted-foreground/40 tabular-nums shrink-0">{log.timestamp}</span>
                
                {/* Source tag */}
                <span className={cn(
                  "px-1.5 py-0.2 rounded-none border text-[8px] font-bold tracking-widest uppercase shrink-0 w-16 text-center",
                  log.source === "ENGINE" ? "bg-blue-500/10 border-blue-500/20 text-blue-400" :
                  log.source === "SCRAPER" ? "bg-amber-500/10 border-amber-500/20 text-amber-400" :
                  log.source === "VALIDATOR" ? "bg-red-500/10 border-red-500/20 text-red-400" :
                ...
                <span className={cn(
                  ...
                  log.type === "CRITICAL" ? "text-red-400 font-extrabold" :
