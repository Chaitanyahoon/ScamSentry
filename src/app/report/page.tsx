      ...
      <div className="container px-4 sm:px-6 lg:px-8 max-w-4xl mx-auto relative z-10">
        ...
        <div className="text-center mb-10 max-w-xl mx-auto">
          ...
          <p className="text-sm text-muted-foreground mt-2">
            Help protect the community. Log the details of job scams, phishing websites, or other fraudulent links.
          </p>
          
          {/* Progress Stepper */}
          <div className="mt-8 flex items-center justify-between text-xs text-muted-foreground font-semibold">
            <span className={step >= 1 ? "text-primary font-bold" : ""}>1. Category</span>
            <div className="flex-1 h-px bg-border mx-4 relative" />
            <span className={step >= 2 ? "text-primary font-bold" : ""}>2. Details</span>
            <div className="flex-1 h-px bg-border mx-4 relative" />
            <span className={step >= 3 ? "text-primary font-bold" : ""}>3. Submit</span>
          </div>
        </div>
        ...
        <div className="bg-card/50 backdrop-blur-sm border border-border shadow-xl rounded-2xl overflow-hidden relative">
          ...
          <div className="p-6 sm:p-10">
            ...
            {step === 1 && (
              <div className="space-y-6 animate-in fade-in slide-in-from-right-4 duration-300">
                <div className="mb-4 border-b border-border pb-4">
                  <h2 className="text-lg font-bold text-foreground tracking-tight">Step 1: Select Scam Category</h2>
                  <p className="text-xs text-muted-foreground">Select the type of fraud classification that best describes the scam.</p>
                </div>
                  ...
                  {scamTypes.map((type) => {
                    ...
                    return (
                      ...
                      >
                        ...
                        <p className="text-xs text-muted-foreground/85 mt-3 leading-relaxed">{type.description}</p>
                      </div>
                    )
                  })}
                </div>
            ...
            {step === 2 && (
              <div className="space-y-6 animate-in fade-in slide-in-from-right-4 duration-300">
                <div className="mb-4 border-b border-border pb-4">
                  <h2 className="text-lg font-bold text-foreground tracking-tight">Step 2: Incident Details</h2>
                  <p className="text-xs text-muted-foreground">Provide details about the scam to help identify and record this incident.</p>
                </div>

                <div className="space-y-6 bg-muted/10 border border-border p-6 rounded-xl">
                  <div>
                    <Label htmlFor="title" className="text-xs font-semibold text-foreground mb-2 block">Report Title *</Label>
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
                    <div>
                      <Label htmlFor="company" className="text-xs font-semibold text-foreground mb-2 block">Company / Entity Name</Label>
                      <div className="relative">
                    ...
                    <div>
                      <Label htmlFor="industry" className="text-xs font-semibold text-foreground mb-2 block">Industry Classification</Label>
                      <Select
                      ...
                      >
                        ...
                        <SelectContent className="bg-card border-border text-foreground rounded-xl">
                          {industries.map((ind) => (
                            <SelectItem key={ind} value={ind} className="text-xs rounded-lg focus:bg-primary/20 focus:text-primary">{ind}</SelectItem>
                          ))}
                        </SelectContent>
                      </Select>
                    </div>
                  ...
                  <div>
                    <Label htmlFor="location" className="text-xs font-semibold text-foreground mb-2 block">Location (Optional)</Label>
                    <div className="relative">
                  ...
                  <div>
                    <Label htmlFor="description" className="text-xs font-semibold text-foreground mb-2 block">Scam Description *</Label>
                    <Textarea
                      id="description"
                      rows={6}
            ...
            {step === 3 && (
              <div className="space-y-6 animate-in fade-in slide-in-from-right-4 duration-300">
                <div className="mb-4 border-b border-border pb-4">
                  <h2 className="text-lg font-bold text-foreground tracking-tight">Step 3: Verification & Submission</h2>
                  <p className="text-xs text-muted-foreground">Attach supporting files and complete the safety check to submit the report.</p>
                </div>

                <div className="bg-muted/10 border border-border p-6 rounded-xl space-y-6">
                  <div>
                    <Label className="mb-3 block text-xs font-semibold text-foreground">Related Scam Tags</Label>
                    <div className="flex flex-wrap gap-2">
                      {commonTags.map((tag) => (
                        <Badge
                          key={tag}
                          variant="outline"
                          className={cn("cursor-pointer py-1.5 px-3 rounded-full transition-all text-xs font-medium", 
                  ...
                  {/* Evidence Upload */}
                  <div>
                    <Label className="mb-3 block text-xs font-semibold text-foreground">Attach Supporting Evidence (Optional)</Label>
                  ...
                  <div className="border-t border-border pt-6 space-y-4">
                    <div className="flex items-center space-x-3">
                      ...
                      />
                      <Label htmlFor="anonymous" className="cursor-pointer text-sm font-medium text-foreground">
                    ...
                    {!formData.anonymous && (
                      <div className="pl-8 animate-in fade-in duration-200">
                        <Label htmlFor="email" className="text-xs font-semibold text-foreground mb-2 block">Contact Email</Label>
                        <Input
                          id="email"
                          type="email"
