...
export async function GET() {
  try {
    ...
    if (backendUrl) {
      try {
        console.log(`[API] Fetching incidents and brand-lockdowns from FastAPI backend: ${backendUrl}`);
        const [incidentsRes, lockdownsRes] = await Promise.all([
          fetch(`${backendUrl}/api/v1/incidents?limit=30`, { next: { revalidate: 30 } }),
          fetch(`${backendUrl}/api/v1/brand-lockdowns`, { next: { revalidate: 30 } })
        ]);
        ...
        if (incidentsRes.ok && lockdownsRes.ok) {
          ...
          console.log(`[API] Successfully fetched ${incidents.length} incidents and ${lockdowns.length} lockdowns from backend.`);
        } else {
          console.warn(`[API] Backend returned errors: incidents Status=${incidentsRes.status}, lockdowns Status=${lockdownsRes.status}`);
        }
      } catch (err) {
        console.error("[API] Error fetching threat data from backend, falling back to Firestore:", err);
      }
    }
