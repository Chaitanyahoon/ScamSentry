...
export async function POST(req: NextRequest) {
  try {
    ...
    try {
      ...
      return NextResponse.json({ error: "Unauthorized. Invalid authorization token.", details: authErr.message }, { status: 401 });
    }

    // 2. Parse Body
    const body = await req.json().catch(() => null);
    if (!body || !body.webhookUrl || !body.alertType || !body.targetBrand) {
      return NextResponse.json({ error: "Missing required fields: webhookUrl, alertType, targetBrand" }, { status: 400 });
    }
    ...
    try {
      ...
      return NextResponse.json({
        ...
        message: fetchErr.message || "Failed to resolve or connect to the target host."
      });
    }
