  ...
  try {
    ...
    if (backendUrl) {
      try {
        ...
        console.log(`Proxying scan request for: "${targetUrl}" to FastAPI backend: ${backendUrl}`);
        ...
        if (backendResponse.ok) {
          ...
          if (L3.details.matches && Array.isArray(L3.details.matches)) {
            ...
            l3Flags.push(`Warning: Threat Intel check skipped/failed: ${L3.details.error}`);
          }
          ...
          return NextResponse.json(
            {
              ...
              riskLevel,
              forensicReport: {
                layer1_Heuristics: { score: L1.score_contribution, flags: l1Flags },
                layer2_Forensics: { score: L2.score_contribution, flags: l2Flags },
                layer3_ThreatIntel: { score: L3.score_contribution, flags: l3Flags },
                layer4_InternalGraph: { score: L4.score_contribution, flags: l4Flags },
              },
            },
            { status: 200 }
          );
          ...
          console.warn(`Backend responded with error status ${backendResponse.status}: ${errText}. Falling back to local analysis.`);
        }
      } catch (err) {
        console.error("Error connecting to backend. Falling back to local analysis:", err);
      }
    }
