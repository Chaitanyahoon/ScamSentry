...
export default function ReportDetailPage() {
  ...
  const { isOsint, sourceName, badgeColor, textColor, brandColor, sourceLink } =
...
function getSourceDetails(report: ScamReport) {
  const isOsint =
    report.title.toUpperCase().startsWith("OSINT:") ||
    report.company === "OSINT Threat Feed";
  const sourceLink =
    report.evidenceUrls && report.evidenceUrls.length > 0
  ...
  if (isOsint) {
    if (sourceLink) {
      ...
      if (urlLower.includes("bleepingcomputer.com")) {
        ...
        urlLower.includes("thehackernews.com") ||
        urlLower.includes("feedburner")
      ) {
