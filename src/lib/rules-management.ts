...
export async function createRule(
  rule: Omit<DetectionRule, "id" | "lastUpdated" | "detectionCount">,
  userId: string,
): Promise<string> {
  try {
    const newRule = {
 ...
 */
export async function enableCategoryRules(
  category: string,
  userId: string,
): Promise<void> {
  try {
    ...
    const promises = rules.map((rule) =>
      updateRule(rule.id!, { enabled: true }, userId),
    );
 ...
 */
export async function disableCategoryRules(
  category: string,
  userId: string,
): Promise<void> {
  try {
    const rules = await getRulesByCategory(category);
    const promises = rules.map((rule) =>
      updateRule(rule.id!, { enabled: false }, userId),
    );
    await Promise.all(promises);
  } catch (error) {
 ...
 */
export async function getLayerConfig(
  layerName: string,
