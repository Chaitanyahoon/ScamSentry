export const MALICIOUS_URL_DICTIONARIES = {
  ...
  embeddedRedirect:
    /@|%40|\/(?:goto|redirect|info|download|click|action|callback|confirm|validate)\//i,
  credentialHarvesting:
    /password|passwd|pwd|secret|token|key|credential|auth|login|username|email/i,
...
export const EMAIL_PATTERN_DICTIONARY = {
  ...
  freeMailProviders:
    /^(gmail|yahoo|outlook|hotmail|aol|zoho|protonmail|yandex|mail|gmx|icloud)\.com$/i,
};
...
const HOMOGLYPH_MAP: Record<string, string> = {
  ...
  ｕ: "u",
  "0": "o",
  "1": "l", // Common number-swaps included for normalization
};
...
export function analyzeHeuristics(inputUrl: string): {
  ...
  if (emailMatch) {
    ...
    const isFreeProvider =
      EMAIL_PATTERN_DICTIONARY.freeMailProviders.test(emailDomain);
  ...
  const isTrustedAppDomain = TRUSTED_APP_DOMAINS.some(
  ...
  try {
    ...
    if (originalHostname !== normalizedHostname) {
      ...
      const isSpoofingBrand = TARGET_BRANDS.some(
        (brand) =>
