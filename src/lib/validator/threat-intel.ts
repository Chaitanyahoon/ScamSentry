export async function analyzeThreatIntel(urls: string[]) {
  ...
  if (!apiKey) {
    flags.push(
      "Threat Intel: Safe Browsing checks skipped (API Key not configured).",
    );
  ...
  try {
    ...
    if (response.ok) {
      ...
      if (data.matches && data.matches.length > 0) {
        ...
        const maliciousUrls = Array.from(
          new Set(data.matches.map((m: any) => m.threat.url)),
        );
        flags.push(
          `CRITICAL: Google Safe Browsing flagged these URLs as malicious: ${maliciousUrls.join(", ")}`,
        );
      } else {
