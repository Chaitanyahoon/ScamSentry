...
export async function syncOSINTFeeds() {
  ...
  try {
    ...
    try {
      ...
      console.warn(
        "[OSINT] Graceful skip: Firebase Admin environment variables are not configured.",
      );
      return {
        processed: 0,
        added: 0,
    ...
    try {
      ...
      const openPhishDomains = openPhishText
        .split("\n")
        .map((line) => line.trim())
      ...
      const uniqueOpenPhish = Array.from(new Set(openPhishDomains)).slice(
        0,
        250,
      );
    ...
    try {
      ...
      const phishTankDomains = phishTankData
        .map((item: any) => {
          try {
      ...
      const uniquePhishTank = Array.from(new Set(phishTankDomains)).slice(
        0,
        250,
      );
...
async function processDomain(
  ...
  try {
    ...
    if (existing.exists) {
      ...
      const lastSyncTime = data?.lastSync?.toDate
        ? data.lastSync.toDate().getTime()
        : 0;
 ...
 */
export async function getThreatDossier(
  domainOrId: string,
): Promise<OSINTThreat | null> {
  try {
    const db = getAdminDb();
