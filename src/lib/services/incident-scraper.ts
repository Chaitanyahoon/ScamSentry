...
export async function scrapeCyberIncidents() {
  ...
  try {
    ...
    try {
      ...
      console.warn(
        "[INCIDENT_SCRAPER] Graceful skip: Firebase Admin not configured.",
      );
      return stats;
    }
    ...
    for (const feed of ADVISORY_FEEDS) {
      try {
        console.log(
          `[INCIDENT_SCRAPER] Fetching advisory feed: ${feed.name}...`,
        );
        const res = await fetch(feed.url, {
          headers: {
            "User-Agent": "Mozilla/5.0 (ScamSentry-Advisory-Scraper/2.5)",
          },
          cache: "no-store",
        });
        ...
        console.log(
          `[INCIDENT_SCRAPER] Parsing ${itemMatches.length} advisory reports from ${feed.name}...`,
        );
        ...
        for (const itemXml of itemMatches.slice(0, 20)) {
          ...
          for (const brand of MONITORED_BRANDS) {
            ...
            if (hasBrand && isCompromised) {
              ...
              console.log(
                `[INCIDENT_SCRAPER] 🚨 BRAND LOCKDOWN ACTIVE: '${brand.toUpperCase()}' due to incident: "${title}"`,
              );
        ...
        console.error(
          `[INCIDENT_SCRAPER] Failed to process feed ${feed.name}:`,
          feedError,
        );
        stats.errors++;
      }
 ...
 */
export async function generateCommunityReportsFromIncidents(): Promise<{
  ...
  try {
    ...
    console.error(
      "[INCIDENT_SCRAPER] Failed to generate community reports:",
      e,
    );
  }
...
export async function getActiveBrandLockdowns(): Promise<string[]> {
  try {
    ...
    const snapshot = await db
      .collection("active_brand_lockdowns")
      .where("expiresAt", ">", now)
      .where("isActive", "==", true)
      .get();
